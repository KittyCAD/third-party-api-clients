...
impl Job {
    ...
    pub fn list_codes_stream<'a>(
        ...
        let pagination_url_path = ("job-codes").to_string();
        ...
        let stream = self
            .list_codes(None, expand, filter, order_by)
            .map_ok(move |result| {
    ...
    pub fn list_dimensions_stream<'a>(
        ...
        use crate::types::paginate::Pagination;
        let pagination_url_path = ("job-dimensions").to_string();
        let mut pagination_query_params: Vec<(&str, String)> = Vec::new();
        if let Some(p) = filter.as_ref() {
            pagination_query_params.push(("filter", p.clone()));
        }
        ...
        let stream = self
            .list_dimensions(None, filter, order_by)
            .map_ok(move |result| {
                let items = futures::stream::iter(result.items().into_iter().map(Ok));
                let next_pages = futures::stream::try_unfold(
                    (None, result),
