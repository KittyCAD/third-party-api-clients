...
impl Contacts {
    ...
    #[doc = "Merge contacts\n\nMerges the contacts specified into a single contact, deleting the \
             merged-in contacts.\nIf a target contact ID is supplied, the other contacts will be \
             merged into that one.\nOtherwise, some contact in the contact ID list will be treated \
             ...
             will be concatenated and separated by newlines in order from\noldest to newest with \
             the (optional) target contact's description first\n* all handles, groups, links, and \
             notes will be preserved\n* other conflicts will use the most recently updated \
             contact's value\n\n\n```rust,no_run\nasync fn example_contacts_merge() -> \
             anyhow::Result<()> {\n    let client = front_api::Client::new_from_env();\n    let \
             result: front_api::types::ContactResponse = client\n        .contacts()\n        \
             .merge(&front_api::types::MergeContacts {\n            target_contact_id: \
             Some(\"some-string\".to_string()),\n            contact_ids: \
