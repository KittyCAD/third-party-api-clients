...
impl Client {
    ...
    /// Set the base URL for the client to something other than the default: <https://us1.api.mailchimp.com>.
    #[tracing::instrument]
        ...
        R: ToString + std::fmt::Debug,
    {
        ...
        let client_secret = env::var("MAILCHIMP_CLIENT_SECRET").expect("must set MAILCHIMP_CLIENT_SECRET");
        let redirect_uri = env::var("MAILCHIMP_REDIRECT_URI").expect("must set MAILCHIMP_REDIRECT_URI");
    ...
    pub fn user_consent_url(&self, scopes: &[String]) -> String {
        ...
        let url = format!(
            "https://login.mailchimp.com/oauth2/authorize?client_id={}&response_type=code&redirect_uri={}&state={}",
...
/// Return a reference to an interface that provides access to root operations.
               pub fn root(&self) -> root::Root {
                    root::Root::new(self.clone())
               }

/// Return a reference to an interface that provides access to activityFeed operations.
               pub fn activity_feed(&self) -> activity_feed::ActivityFeed {
                    activity_feed::ActivityFeed::new(self.clone())
               }

/// Return a reference to an interface that provides access to accountExports operations.
               pub fn account_exports(&self) -> account_exports::AccountExports {
                    account_exports::AccountExports::new(self.clone())
               }

/// Return a reference to an interface that provides access to accountExport operations.
               pub fn account_export(&self) -> account_export::AccountExport {
                    account_export::AccountExport::new(self.clone())
               }

/// Return a reference to an interface that provides access to authorizedApps operations.
               pub fn authorized_apps(&self) -> authorized_apps::AuthorizedApps {
                    authorized_apps::AuthorizedApps::new(self.clone())
               }

/// Return a reference to an interface that provides access to automations operations.
               pub fn automations(&self) -> automations::Automations {
                    automations::Automations::new(self.clone())
               }

/// Return a reference to an interface that provides access to batches operations.
               pub fn batches(&self) -> batches::Batches {
                    batches::Batches::new(self.clone())
               }

/// Return a reference to an interface that provides access to batchWebhooks operations.
               pub fn batch_webhooks(&self) -> batch_webhooks::BatchWebhooks {
                    batch_webhooks::BatchWebhooks::new(self.clone())
               }

/// Return a reference to an interface that provides access to templateFolders operations.
               pub fn template_folders(&self) -> template_folders::TemplateFolders {
                    template_folders::TemplateFolders::new(self.clone())
               }

/// Return a reference to an interface that provides access to campaignFolders operations.
               pub fn campaign_folders(&self) -> campaign_folders::CampaignFolders {
                    campaign_folders::CampaignFolders::new(self.clone())
               }

/// Return a reference to an interface that provides access to campaigns operations.
               pub fn campaigns(&self) -> campaigns::Campaigns {
                    campaigns::Campaigns::new(self.clone())
               }

/// Return a reference to an interface that provides access to connectedSites operations.
...
/// Return a reference to an interface that provides access to customerJourneys operations.
               pub fn customer_journeys(&self) -> customer_journeys::CustomerJourneys {
                    customer_journeys::CustomerJourneys::new(self.clone())
               }

/// Return a reference to an interface that provides access to fileManager operations.
               pub fn file_manager(&self) -> file_manager::FileManager {
                    file_manager::FileManager::new(self.clone())
               }

/// Return a reference to an interface that provides access to lists operations.
               pub fn lists(&self) -> lists::Lists {
                    lists::Lists::new(self.clone())
               }

/// Return a reference to an interface that provides access to Surveys operations.
               pub fn surveys(&self) -> surveys::Surveys {
                    surveys::Surveys::new(self.clone())
               }

/// Return a reference to an interface that provides access to landingPages operations.
               pub fn landing_pages(&self) -> landing_pages::LandingPages {
                    landing_pages::LandingPages::new(self.clone())
               }

/// Return a reference to an interface that provides access to reports operations.
               pub fn reports(&self) -> reports::Reports {
                    reports::Reports::new(self.clone())
               }

/// Return a reference to an interface that provides access to templates operations.
               pub fn templates(&self) -> templates::Templates {
                    templates::Templates::new(self.clone())
               }

/// Return a reference to an interface that provides access to ecommerce operations.
               pub fn ecommerce(&self) -> ecommerce::Ecommerce {
                    ecommerce::Ecommerce::new(self.clone())
               }

/// Return a reference to an interface that provides access to searchCampaigns operations.
               pub fn search_campaigns(&self) -> search_campaigns::SearchCampaigns {
                    search_campaigns::SearchCampaigns::new(self.clone())
               }

/// Return a reference to an interface that provides access to searchMembers operations.
               pub fn search_members(&self) -> search_members::SearchMembers {
                    search_members::SearchMembers::new(self.clone())
               }

/// Return a reference to an interface that provides access to ping operations.
               pub fn ping(&self) -> ping::Ping {
                    ping::Ping::new(self.clone())
               }

/// Return a reference to an interface that provides access to facebookAds operations.
               pub fn facebook_ads(&self) -> facebook_ads::FacebookAds {
                    facebook_ads::FacebookAds::new(self.clone())
               }

/// Return a reference to an interface that provides access to reporting operations.
               pub fn reporting(&self) -> reporting::Reporting {
                    reporting::Reporting::new(self.clone())
               }

/// Return a reference to an interface that provides access to verifiedDomains operations.
