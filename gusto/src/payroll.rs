...
impl Payroll {
    ...
    #[doc = "Get all payrolls for a company\n\nReturns all payrolls, current and past for a \
             company.\n\nNotes:\n* Hour and dollar amounts are returned as string representations \
             of numeric decimals.\n* Hours are represented to the thousands place; dollar amounts \
             are represented to the cent.\n* Every eligible compensation is returned for each \
             employee. If no data has yet be inserted for a given field, it defaults to “0.00” \
             (for fixed amounts) or “0.000” (for hours ).\n\n**Parameters:**\n\n- \
             `company_id_or_uuid: &'astr`: The ID or UUID of the company (required)\n- `end_date: \
             Option<chrono::NaiveDate>`: Return payrolls whose pay period is before the end \
             date\n- `include: Option<Vec<crate::types::GetCompaniesCompanyIdPayrollsInclude>>`: \
             Include the requested attribute in the employee_compensations attribute in the \
             response\n- `include_off_cycle: Option<bool>`: Whether to include off cycle payrolls \
             in the response\n- `processed: Option<bool>`: Whether to return processed or \
             unprocessed payrolls\n- `start_date: Option<chrono::NaiveDate>`: Return payrolls \
             whose pay period is after the start date\n\n```rust,no_run\nasync fn \
             example_payroll_get_companies_company_id_payrolls() -> anyhow::Result<()> {\n    let \
             client =\n        gusto_api::Client::new_from_env(String::from(\"token\"), \
             String::from(\"refresh-token\"));\n    let result: Vec<gusto_api::types::Payroll> = \
             client\n        .payroll()\n        .get_companies_company_id_payrolls(\n            \
             \"some-string\",\n            Some(chrono::Utc::now().date_naive()),\n            \
             Some(vec![\n                \
             gusto_api::types::GetCompaniesCompanyIdPayrollsInclude::Deductions,\n            \
             ]),\n            Some(true),\n            Some(true),\n            \
             Some(chrono::Utc::now().date_naive()),\n        )\n        .await?;\n    \
             println!(\"{:?}\", result);\n    Ok(())\n}\n```"]
    #[tracing::instrument]
